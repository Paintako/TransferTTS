...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def _filter(self):
        ...
        lengths = []
        for audiopath, text, sid in self.audiopaths_sid_text:
            audiopath = os.path.join(
                self.data_path, 'wav{}'.format(self.sampling_rate // 1000),
                sid, '{}.wav'.format(audiopath))
            if self.min_text_len <= len(text) and len(
                    text) <= self.max_text_len:
    ...
    def get_audio_text_speaker_pair(self, audiopath_sid_text):
        ...
        audiopath, text, sid = audiopath_sid_text[0], audiopath_sid_text[
            1], audiopath_sid_text[2]

        # raise ValueError('sotp')
        text = self.get_text(text)
    ...
    def get_audio(self, filename, sid):
        # filename = os.path.join(self.data_path, 'wav{}'.format(self.sampling_rate//1000), sid, '{}.wav'.format(filename))
        filename = os.path.join(self.data_path,
                                'wav{}'.format(self.sampling_rate // 1000),
                                sid, filename)
