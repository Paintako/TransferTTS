...
class Conv1dGLU(nn.Module):
  ...
  '''
    ...
    def __init__(self, in_channels, out_channels, kernel_size, dropout):
        ...
        self.out_channels = out_channels
        self.conv1 = ConvNorm(in_channels,
                              2 * out_channels,
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        q = q.permute(2, 0, 1, 3).contiguous().view(-1, len_x,
                                                    d_k)  # (n*b) x lq x dk
        k = k.permute(2, 0, 1, 3).contiguous().view(-1, len_x,
                                                    d_k)  # (n*b) x lk x dk
        v = v.permute(2, 0, 1, 3).contiguous().view(-1, len_x,
                                                    d_v)  # (n*b) x lv x dv
