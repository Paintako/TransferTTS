...
class SynthesizerTrn(nn.Module):
  ...
  """
    ...
    '''
        ...
        #* Zero-shot
        style_vector = self.style_encoder(mel.transpose(1, 2),
                                          (y_mask.int() == 0).squeeze(1))
    ...
    def infer(self,
        ...
        if mel_lengths is not None:
            style_mask = torch.unsqueeze(
                commons.sequence_mask(mel_lengths, mel.size(2)), 1).to(x.dtype)
            style_vector = self.style_encoder(mel.transpose(
                1, 2), (style_mask.int() == 0).squeeze(1))
        else:
...
class MelStyleEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.spectral = nn.Sequential(
            modules.LinearNorm(self.in_dim, self.hidden_dim), modules.Mish(),
            nn.Dropout(self.dropout),
            modules.LinearNorm(self.hidden_dim, self.hidden_dim),
            modules.Mish(), nn.Dropout(self.dropout))
    ...
    def forward(self, x, mask=None):
        max_len = x.shape[1]
        slf_attn_mask = mask.unsqueeze(1).expand(
            -1, max_len, -1) if mask is not None else None
